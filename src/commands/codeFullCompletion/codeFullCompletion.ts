...
export async function codeFullCompletion() {
  ...
  statusBarItem.text = '$(sync-spin) ✨emAI：思考中...'
  statusBarItem.show()
  ...
  try {
    ...
    console.log('✨ emAI：system提示语', systemPrompt)
    ...
    console.log('✨emAI：用户提示语', userPrompt)
    ...
    if (resp?.choices?.[0]?.message?.content) {
      ...
      if (code) {
        ...
        vscode.window.showErrorMessage('✨emAI：此次AI生成代码位置异常，请重试')
        return null
      }
    }
  } catch (error) {
    vscode.window.showErrorMessage('✨ emAI：与AI的对话异常')
    console.error('✨emAI：与AI的对话异常', error)
  } finally {
